...
class CrawlThread(threading.Thread):
    ...
    def __init__(self, thread_id, queue, limit, topic):
        super().__init__()
    ...
    def scheduler(self):
        while not self.queue.empty():
            ...
            url = f'https://www.zhihu.com/api/v4/topics/{self.topic}/best_answerers?limit={self.limit}'
...
if __name__ == '__main__':
    ...
    for thread_id in crawl_name_list:
        thread = CrawlThread(thread_id, pageQueue, 15, topicID)
        thread.start()
        crawl_threads.append(thread)

    # 将结果保存到一个json文件中
    filePath = Path(os.getcwd()).resolve().joinpath( topicID + '_article.txt')
